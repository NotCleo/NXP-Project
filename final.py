...
class WarehouseExplore(Node):
    ...
    def __init__(self):
        ...
        # --- Shelf Goal Queue ---
        self.shelf_goal_queue = []
        self.shelf_goal_index = 0
        self.shelf_queue_enabled = False
    ...
    def detect_shelves_from_map(self):
        ...
        # -------------------------
        # BUILD SHELF GOAL QUEUE
        # -------------------------
        self.shelf_goal_queue.clear()

        for qr_pose, obj_pose in self.shelf_goal_poses:
            self.shelf_goal_queue.append(qr_pose)
            self.shelf_goal_queue.append(obj_pose)

        self.shelf_goal_index = 0
        self.shelf_queue_enabled = True
        self.logger.info(f"Initialized shelf goal queue with {len(self.shelf_goal_queue)} poses.")

        # Optionally trigger first goal automatically:
        self.send_next_goal_from_queue()
    ...
    def camera_image_callback(self, message):
        # Decode the compressed ROS image
        np_arr = np.frombuffer(message.data, np.uint8)
        image = cv2.imdecode(np_arr, cv2.IMREAD_COLOR)

        # ---- QR Detection via pyzbar ----
        decoded_objects = pyzbar.decode(image)

        if decoded_objects:
            # Take only the first detected QR code for this example
            self.qr_code_str = decoded_objects[0].data.decode("utf-8")
            qr_str = self.qr_code_str

            try:
                # FIX: Check format before unpacking
                parts = qr_str.split("_")
                if len(parts) != 3:
                    self.get_logger().error(f"Invalid QR format: {qr_str}")
                else:
                    shelf_id_str, angle_str, secret_code = parts
                    shelf_id = int(shelf_id_str)
                    angle_deg = float(angle_str)
                    angle_rad = np.deg2rad(angle_deg)

                    # Save decoded angle for use after goal completes
                    self.next_shelf_angle_rad = angle_rad
                    self.next_shelf_secret_code = secret_code
                    self.next_shelf_id = shelf_id

                    self.get_logger().info(
                        f"[DEBUG] Parsed QR: shelf_id={shelf_id}, angle={angle_deg:.2f}, secret={secret_code}"
                    )

                    self.visited_shelf_ids.add(shelf_id)
                    self.logger.info(f"Visited shelves so far: {self.visited_shelf_ids}")

            except Exception as e:
                self.get_logger().error(f"Failed parsing QR string: {qr_str}. Error: {e}")

            self.get_logger().info(
                f"[INFO] QR Code(s) FOUND: {[obj.data.decode('utf-8') for obj in decoded_objects]}"
            )
            print(f"[DEBUG] QR code decoded: {self.qr_code_str}")
            ...
            self.qr_code_str = "Empty"
            self.get_logger().info("[INFO] No QR code detected.")
            print("[DEBUG] No QR code detected.")
            if PROGRESS_TABLE_GUI:
                box_app.change_box_text(
                    self.table_row_count + 1, self.table_col_count, "No QR"
                )
        ...
        for pred in output_data:
            ...
            detections = self.non_max_suppression(
                torch_pred, 0.25, 0.45, max_det=1000
            )
            for det in detections:
                ...
                for *xyxy, conf, cls in det:
                    label = (
                        self.label_names[int(cls)]
                        if int(cls) < len(self.label_names)
                        else str(int(cls))
                    )
                    detected_objects[label] = (
                        detected_objects.get(label, 0) + 1
                    )
        ...
        if detected_objects:
            ...
            self.get_logger().info(
                f"[DEBUG] Published shelf data: {shelf_data_message}"
            )
        ...
        if PROGRESS_TABLE_GUI:
            ...
            box_app.change_box_text(
                self.table_row_count, self.table_col_count, obj_str
            )
            box_app.change_box_color(
                self.table_row_count, self.table_col_count, "cyan"
            )

            box_app.change_box_text(
                self.table_row_count + 1,
                self.table_col_count,
                self.qr_code_str,
            )
            box_app.change_box_color(
                self.table_row_count + 1, self.table_col_count, "yellow"
            )
    ...
    def goal_result_callback(self, future):
        ...
        if status == GoalStatus.STATUS_SUCCEEDED:
            ...
            # Check if we have decoded a heuristic angle
            if hasattr(self, "next_shelf_angle_rad"):
                distance_to_next_shelf = 3.0  # meters
                angle = self.next_shelf_angle_rad

                next_x = self.buggy_pose_x + distance_to_next_shelf * np.cos(angle)
                next_y = self.buggy_pose_y + distance_to_next_shelf * np.sin(angle)

                self.logger.info(
                    f"Computed next shelf position at ({next_x:.2f}, {next_y:.2f}) from heuristic angle {np.rad2deg(angle):.2f}°"
                )

                # Create next goal
                next_goal_pose = self.create_goal_from_world_coord(next_x, next_y, angle)

                # Dispatch navigation to next shelf
                success = self.send_goal_from_world_pose(next_goal_pose)
                if success:
                    self.logger.info("Dispatched goal towards next shelf.")
                    # After goal is dispatched:
                    del self.next_shelf_angle_rad
                    del self.next_shelf_secret_code
                    del self.next_shelf_id

                else:
                    self.logger.error("Failed to dispatch goal towards next shelf.")
        ...
        # Move to next goal if we are in queue mode
        if self.shelf_queue_enabled:
            self.shelf_goal_index += 1
            self.send_next_goal_from_queue()
    ...
    def send_goal_from_world_pose(self, goal_pose):
        ...
        return True
    def send_next_goal_from_queue(self):
        """
        Pops the next goal from the queue and sends it to Nav2.
        """
        if not self.shelf_queue_enabled:
            self.logger.info("Shelf queue traversal is disabled.")
            return

        if self.shelf_goal_index >= len(self.shelf_goal_queue):
            self.logger.info("Shelf queue traversal completed!")
            self.shelf_queue_enabled = False
            return

        next_goal = self.shelf_goal_queue[self.shelf_goal_index]
        self.logger.info(
            f"Sending goal {self.shelf_goal_index + 1}/{len(self.shelf_goal_queue)} "
            f"at x={next_goal.pose.position.x:.2f}, y={next_goal.pose.position.y:.2f}"
        )
        success = self.send_goal_from_world_pose(next_goal)
        if success:
            self.logger.info(f"Goal {self.shelf_goal_index + 1} dispatched successfully.")
        else:
            self.logger.error(f"Failed to dispatch goal {self.shelf_goal_index + 1}.")
